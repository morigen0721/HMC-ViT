...
class Conv_Layers(nn.Module):
    def __init__(self,in_channels: int = 3):
        ...
        self.conv1 = nn.Conv2d(in_channels, in_channels * 4, (1,3), stride = 1, padding = (0,1))  #参数：输入通道数，输出通道数，卷积核大小，步长，填充
        self.conv2 = nn.Conv2d(in_channels * 4, in_channels, (1,3), stride = 1, padding = (0,1))  #卷积核大小、步长、填充确保输出的空间尺寸与输入相同
    ## 定义前向传播方法
...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self, in_channels: int = 3, patch_size_x: int = 1, patch_size_y: int = 6, emb_size: int = 64, img_size: int = 120):
        ## 把patch尺寸保存到类的属性中
        self.patch_size_x = patch_size_x
        self.patch_size_y = patch_size_y
        ## 计算图像中patch的数量，加1是为了加入一个CLS token。这里假设了可以整除
        self.positions_num = (img_size // self.patch_size_y) + 2
        super().__init__()
        ...
        self.projection = nn.Sequential(
            ...
            ## 原始图像为in_channels通道，输出的特征图为emb_size个，emb_size也就是卷积核的个数
            nn.Conv2d(in_channels, emb_size, kernel_size = (patch_size_x, patch_size_y), stride = (1, 6)),
...
class ClassificationHead(nn.Module):
    def __init__(self, emb_size=64, num_classes=121, dropout_rate=0.25):
        super(ClassificationHead, self).__init__()
        self.fc1 = nn.Linear(emb_size * 2, emb_size * 8)
        self.fc2 = nn.Linear(emb_size * 8, emb_size * 4)
        self.final_fc = nn.Linear(emb_size * 4, num_classes)
...
class tricnndualtran(nn.Sequential):
    def __init__(self,     
