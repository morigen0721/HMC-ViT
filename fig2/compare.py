...
from partricnntran import *
from cnndualtran import *
from tricnndualtran import *
import torch
from toolbox import *
import cvxpy as cp
...
def test_dual_tran(data, sig_num, real_doa):
    # Path to model parameters
    parameter_path = '/home/yxxie/Documents/mutli_cls_tran_cnn/fig2_Ablation/draw/dualtran.pth'
...
def test_par_tri_cnn_tran(data, sig_num, real_doa):
    # Path to model parameters
    parameter_path = '/home/yxxie/Documents/mutli_cls_tran_cnn/fig2_Ablation/draw/parteicnntran.pth'
...
def test_tricnndualtran(data, sig_num, real_doa):
    # Path to model parameters
    parameter_path = '/home/yxxie/Documents/mutli_cls_tran_cnn/fig2_Ablation/draw/tricnndualtran.pth'
    ...
    sample = batch_extract_upper_triangle(data).view(data.shape[0], 3, 1, 120)
    

    # Setup and load the model
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = tricnndualtran()
    model.load_state_dict(torch.load(parameter_path, map_location=device))
    model.to(device)
    model.eval()
    sample = sample.to(device)  # Move input data to GPU
...
def test_cnndualtran(data, sig_num, real_doa):
    # Path to model parameters
    parameter_path = '/home/yxxie/Documents/mutli_cls_tran_cnn/fig2_Ablation/draw/cnndualtran.pth'
...
## Note: If angle range is -90 to 90 or 0 to 180, ensure MUSIC is adjusted accordingly
...
def Music(data, array_num, sig_num, lamda, angle_gap):
    ...
    grid_angle = np.arange(0, 180.0001, angle_gap)  # Angle grid, generates a grid of length 180/angle_gap + 1
    ...
    for i in range(len(grid_angle)):
        ##### Pay attention to handling angle range, sincos, angle adjustment (e.g., subtracting 90), etc.
        ...
        if np.abs(W) > 1e-10:
            ...
            Pmusic[i] = 0.0  # Set this point to 0 and wait for special handling later
