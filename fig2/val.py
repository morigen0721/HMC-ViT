...
data_dir = '/home/yxxie/Documents/mutli_cls_tran_cnn/fig2_Ablation/val_data'  # Path to test data
num_cpus = 1
...
result_dir = '/home/yxxie/Documents/mutli_cls_tran_cnn/fig2_Ablation/val_result'  # Path to save results
method_name_list = ['test_par_tri_cnn_tran'] # Specify the method names for performance comparison
method_name_list = ['MC-ViT'] # Specify the method names for performance comparison
method_type = 'DL'  # Specify method type, 'DL' for deep learning methods, others for traditional methods
...
## Processing function, reads .npz files and computes results
def process_file(file):
    ...
    # Extract data conditions and convert to non-numpy arrays for pandas compatibility
    data = meta_data['data_cov']  # Extract covariance matrix data
    sig_num = meta_data['sig_num'].astype(int).item()  # Extract number of sources
    doa = tuple(meta_data['doa'])  # Extract actual DOA
    snr = meta_data['snr'].item()  # Extract signal-to-noise ratio
    snap_num = meta_data['snap_num'].item()  # Extract number of snapshots
    RMSE = 0  # Initialize RMSE

    ## If the evaluation method is a traditional method, process each Monte Carlo experiment and compute RMSE
    if method_type != 'DL':
        ...
        for i in range(data.shape[0]):
            ...
            RMSE += get_MSE(est_doa, meta_data['doa'])  # Compute RMSE for this Monte Carlo experiment and accumulate
    
    ## If the evaluation method is a deep learning method, use GPU for acceleration
    ...
    ## Assemble the experimental results for this condition
    result = {
    ...
    with lock:
        counter.value += 1
        print(f"Processed {counter.value} conditions, took {elapsed_time:.4f} seconds for 1000 Monte Carlo experiments for this condition.")
...
if __name__ == '__main__':
    ...
    for i in range(len(method_name_list)):
        ...
        ## Check if the experimental results have already been computed
        if not os.path.exists(csv_file_path):  # If not, call the process_file function to compute results in parallel
            # Use multi-processing to compute experimental results
            with concurrent.futures.ProcessPoolExecutor(max_workers=num_cpus) as executor:
                results = list(executor.map(process_file, files))
            os.makedirs(folder_path, exist_ok=True)
            # Organize the experimental results into a DataFrame
