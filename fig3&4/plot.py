    ...
    for sig_num in sorted(all_sig_nums):
        for snr in snr_range:
            for snapshot in snapshot_range:
                ...
                plot_order = ["Music", "CNN", "DCT-ViT", "MC-ViT"]
                method_markers = {
                    "MC-ViT":"^",
                    "DCT-ViT":"s",
                    "CNN":"D",
                ...
                for method in plot_order:
                    ...
                    df_filtered = df[(df['sig_num'] == sig_num) & (df['snr'] == snr)]
                    if df_filtered.empty:
                        continue
                    sns.lineplot(data=df_filtered, x='snap_num', y='RMSE', label=method, 
                ...
                plt.yscale('log')
                plt.xlabel(r'Number of Snapshot ($\mathbf{\times 10^2}$)', fontsize=18, fontweight='bold')
                plt.xticks(snapshot_range, [1,2,3,4,5,10,15,20], fontsize=18, fontweight='bold')
                plt.ylabel(r'RMSE ($^\circ$)', fontsize=18, fontweight='bold')
                plt.yticks(fontsize=18, fontweight='bold')
                plt.ylim(0.1, 30)
                plt.legend(
                    title="Method",        # 图例标题
                    fontsize=18,           # 图例中项目的字体大小
                ...
                for method in plot_order:
                    ...
                    if df_filtered.empty:
                        continue
                    sns.lineplot(data=df_filtered, x='snr', y='RMSE', label=method, 
                        marker=method_markers.get(method, 'o'),
                        linewidth=3.5,
                        linestyle='--',
                        markersize=16
                ...
                plt.xticks(snr_range, fontsize=18, fontweight='bold')
                plt.ylabel(r'RMSE ($^\circ$)', fontsize=18, fontweight='bold')
                plt.yticks(fontsize=18, fontweight='bold')
                plt.ylim(0.1, 30)
                plt.legend(
